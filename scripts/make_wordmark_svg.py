...
# ---- geometry / grid ------------------------------------------------------
COLS = int(os.environ.get("WORDMARK_COLS", 88))
ROWS = 0
ROW_MARGIN = int(os.environ.get("WORDMARK_ROW_MARGIN", 4))
CELL_W = 9.3
CELL_H = 15.5
TEXT = os.environ.get("WORDMARK_TEXT", "SEKSAN")
...
def get_system_font(font_size):
    """Auto-detect available bold sans-serif font across Windows, macOS, and Linux."""
    candidates = [
        # Windows Fonts
        os.path.join(os.environ.get("WINDIR", "C:\\Windows"), "Fonts", "arialbd.ttf"),
        os.path.join(os.environ.get("WINDIR", "C:\\Windows"), "Fonts", "segui.ttf"),
...
def emit(frames, mode, out, dur, reveal, username="devSeksan"):
    art_w = COLS * CELL_W
    art_h = ROWS * CELL_H
    canvas_w = 860.0  # บังคับความกว้างให้เท่ากับแถบสถานะ (860px)
    canvas_h = TITLEBAR_H + art_h + PAD * 1.5
    art_left = (canvas_w - art_w) / 2.0
    ...
    p = [
        ...
        '</linearGradient></defs>',
        f'<rect width="{canvas_w:.0f}" height="{canvas_h:.0f}" rx="10" fill="url(#wbg)"/>',
        f'<rect x="0.5" y="0.5" width="{canvas_w-1:.0f}" height="{canvas_h-1:.0f}" rx="10" '
    ...
    p.append(f'<clipPath id="wipe"><rect x="{art_left:.1f}" y="{art_top:.1f}" height="{art_h:.1f}" width="0">'
    ...
    p.append(f'<rect x="{art_left:.1f}" y="{art_top+2:.1f}" width="{CELL_W*2.0:.1f}" height="{art_h-4:.1f}" '
             f'fill="{INK}" opacity="0.16">'
             f'<animate attributeName="x" from="{art_left:.1f}" to="{art_left+art_w:.0f}" begin="0s" '
...
def main():
    ...
    ap.add_argument("--reveal", type=float, default=1.5)
    a = ap.parse_args()

    P, N = build_shell()
    rest = math.radians(-11)
    if a.mode == "spin":
        nf = a.frames or 36
    ...
    emit(frames, a.mode, out, dur, a.reveal, username="devSeksan")
